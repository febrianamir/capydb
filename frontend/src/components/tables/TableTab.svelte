<script>
  ...
  import { GetTableColumns, GetTableRecords } from '../../../wailsjs/go/usecase/Usecase'
...
<div class="table-tab {tableTabs.openedTab == tableName ? 'active' : ''}">
  {#if tableColumns.length > 0}
    <TableToolbar
      ...
      lastPage={lastPage}
      queryTableContents={queryTableContents}
      updateQueryTableContents={updateQueryTableContents}
      toggleTableFilter={toggleTableFilter}
      tableColumns={tableColumns}
      tableRecordsCount={tableRecordsCount}
      tableRecordsTotal={tableRecordsTotal}
